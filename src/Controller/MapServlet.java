...
public class MapServlet extends HttpServlet {
	...
	MapOperations mapobject = new MapOperations();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		mapnew = mapobject.add(touristobj);
		
			
		if(add!=null) {
			// call the add method and store the return value in a map variable
			
			request.setAttribute("bucketList", mapobject.add(touristobj));
		...
		if(remove!=null) {
			...
			request.setAttribute("bucketList", mapobject.remove(touristobj));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/map.jsp");
			rd.forward(request, response);
		}
		...
		if(sortRandom!=null) {
			...
			request.setAttribute("bucketList",mapobject.sortRandomly(touristobj));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/map.jsp");
			rd.forward(request, response);
		}
		...
		if(sortInEntryOrder!=null) {
			...
			request.setAttribute("bucketList", mapobject.sortInEntryOrder((HashMap) mapnew));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/map.jsp");
			rd.forward(request, response);
		}
		if(sortAlphabetically!=null) {
			...
			request.setAttribute("bucketList",mapobject.sortAlphabetically((HashMap) mapnew));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/map.jsp");
			rd.forward(request, response);
		}
		...
		if(reset!=null) {	
			...
			request.setAttribute("bucketList", mapobject.reset((HashMap) mapnew));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/map.jsp");
			rd.forward(request, response);
		}
