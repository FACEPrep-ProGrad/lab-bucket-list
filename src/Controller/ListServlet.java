...
public class ListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//creating object for TouristPlace
		...
		List<TouristPlace> list = listobj.add(touristobj);
		...
		if(remove!=null) {
			...
			request.setAttribute("bucketList", listobj.remove(touristobj));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/list.jsp");
			rd.forward(request, response);
		}
		...
		if(sortbydestination!=null) {
			...
			request.setAttribute("bucketList",(List<TouristPlace>)listobj.sortByDestination(list));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/list.jsp");
			rd.forward(request, response);
		}
		...
		if(reset!=null) {	
			...
			request.setAttribute("bucketList", listobj.reset(list));
			RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/list.jsp");
			rd.forward(request, response);
		}
